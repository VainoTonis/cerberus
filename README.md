...
| `callback_url` | Default URL to POST events to when `--callback` is not passed; empty disables (default: disabled). Payloads are always JSON and independent of `--output`, which only controls stdout format |
